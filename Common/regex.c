...
void gkfooo() { return; }
//注意事项，这个版是2005的(已手工更新到2007)，合并成了一个文件，不需要lib和regex2.dll。用lib时用的是2007的，但是需要用c编译器，有问题再换回来
...
# else
	size_t sz = 0;
	_dupenv_s(&codeset_name,&sz,"LC_ALL");//getenv("LC_ALL");
	if (codeset_name == NULL || codeset_name[0] == '\0')
		codeset_name = _dupenv_s(&codeset_name, &sz, "LC_CTYPE");
	if (codeset_name == NULL || codeset_name[0] == '\0')
	//	codeset_name = getenv("LANG");
		_dupenv_s(&codeset_name, &sz, "LANG");
