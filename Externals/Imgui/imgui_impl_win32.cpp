...
{//从主窗消息循环传过来
	...
	{//加，用MainWindow建的窗口，消息绑定没用ImGui_ImplWin32_WndProcHandler_PlatformWindow，窗口变化需要在这通知viewport才能自动调整尺寸和位置
		switch (msg)
		{
		case WM_CLOSE:
    ...
    case WM_MOUSEMOVE:
    {
        ...
        if (!bd->MouseTracked)
        {
            ...
            ::TrackMouseEvent(&tme);//用了它，离开后才会收到WM_MOUSELEAVE
    ...
    case WM_MOUSELEAVE://鼠标离开窗口时发出的消息,必须先调用TrackMouseEvent,
...
struct ImGui_ImplWin32_ViewportData
{
    ...
    DWORD   DwStyle;
    DWORD   DwExStyle;
	MainWindow* window= nullptr;//加，便于删除
...
{//主窗不经过这里，统一用MainWindow建窗口，便于处理调用。可以在自己类中通过修Platform_CreateWindow替换建窗程序
	...
	MainWindow* window = new MainWindow("Kirosoft", rect, vd->DwExStyle, vd->DwStyle);
	vd->window = window;
	vd->Hwnd = window->GetHandle();
...
{//拖出窗口的消循环
#if 0
	WndProcHandlerfromImGui(hWnd,msg,wParam,lParam);//把子窗消息分发到主窗，以便处理点击、拖动等消息
#endif
...
static void ImGui_ImplWin32_InitPlatformInterface()
{
   ...
   // wcex.style = CS_HREDRAW | CS_VREDRAW;
    ...
    wcex.lpszClassName = _T("Kirosoft Platform");
	wcex.hIcon = LoadIcon(GetModuleHandle(NULL), (LPCTSTR)IDI_ICON1);//应用程序的图标
	wcex.hIconSm = LoadIcon(GetModuleHandle(NULL), (LPCTSTR)IDI_ICON1);
...
static void ImGui_ImplWin32_ShutdownPlatformInterface()
{
    ::UnregisterClass(_T("Kirosoft Platform"), ::GetModuleHandle(nullptr));
