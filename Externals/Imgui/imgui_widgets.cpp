...
{
#if SoftType==0||SoftType==0||SoftType==5||SoftType==6
	if (docked_window != NULL)
		return TabTitlebar(tab_bar, label, p_open, flags, docked_window);
#endif
...
void ImGui::DrawTabItem(ImDrawList* draw_list, const ImRect& oRect, ImGuiTabItemFlags flags, ImU32 col, int tabtype)
{
	ImGuiContext& g = *GImGui;
	const float width = oRect.GetWidth();
	IM_UNUSED(flags);
	IM_ASSERT(width > 0.0f);
	const float rounding = ImMax(0.0f, ImMin((flags & ImGuiTabItemFlags_Button) ? g.Style.FrameRounding : g.Style.TabRounding, width * 0.5f - 1.0f));
	const float y1 = oRect.Min.y;
	const float y2 = oRect.Max.y;
	switch (tabtype)
	{
	case 0:
		draw_list->PathLineTo(ImVec2(oRect.Min.x, y2));
		draw_list->PathArcToFast(ImVec2(oRect.Min.x + rounding, y1 + rounding), rounding, 6, 9);
		draw_list->PathArcToFast(ImVec2(oRect.Max.x - rounding, y1 + rounding), rounding, 9, 12);
		draw_list->PathLineTo(ImVec2(oRect.Max.x, y2));
		draw_list->PathFillConvex(col);
		if (g.Style.TabBorderSize > 0.0f)
		{
			draw_list->PathLineTo(ImVec2(oRect.Min.x + 0.5f, y2));
			draw_list->PathArcToFast(ImVec2(oRect.Min.x + rounding + 0.5f, y1 + rounding + 0.5f), rounding, 6, 9);
			draw_list->PathArcToFast(ImVec2(oRect.Max.x - rounding - 0.5f, y1 + rounding + 0.5f), rounding, 9, 12);
			draw_list->PathLineTo(ImVec2(oRect.Max.x - 0.5f, y2));
			draw_list->PathStroke(GetColorU32(ImGuiCol_Border), 0, g.Style.TabBorderSize);
		}
		break;
	case 1:
		draw_list->PathLineTo(ImVec2(oRect.Min.x, y2));
		draw_list->PathArcToFast(ImVec2(oRect.Min.x + rounding, y1 + rounding), rounding, 6, 9);
		draw_list->PathArcToFast(ImVec2(oRect.Max.x-12 - rounding, y1 + rounding), rounding, 9, 12);
		draw_list->PathLineTo(ImVec2(oRect.Max.x, y2));
		draw_list->PathFillConvex(col);
		break;
	case 2:
		draw_list->PathLineTo(ImVec2(oRect.Min.x, y2));
		draw_list->PathArcToFast(ImVec2(oRect.Min.x+12 + rounding, y1 + rounding), rounding, 6, 9);
		draw_list->PathArcToFast(ImVec2(oRect.Max.x -12 - rounding, y1 + rounding), rounding, 9, 12);
		draw_list->PathLineTo(ImVec2(oRect.Max.x, y2));
		draw_list->PathFillConvex(col);
		break;
...
#endif
bool ImGui::TabTitlebar(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
	ImGuiContext& g = *GImGui;
	if (docked_window != NULL)
	{
		int icowidth = 30;
		bool issubwin = (docked_window->DockNode != NULL&&docked_window->DockNode->ParentNode != nullptr && !(docked_window->Pos.y - 10 < docked_window->Viewport->Pos.y&&docked_window->Pos.x + docked_window->Size.x + 10 > docked_window->Viewport->Pos.x + docked_window->Viewport->Size.x));//是否是子窗,只剩一个窗了也当主屏.多屏时放右上角的当主屏保留关闭按钮
		if (issubwin)
		{
			if (g.NextItemData.Width > 0 && g.NextItemData.Width != tab_bar->Tabs[0].Width)
			{
				g.NextItemData.Width = tab_bar->Tabs[0].Width;
				tab_bar->WantLayout = true;
			}
		}
		else if (g.NextItemData.Width + icowidth < docked_window->Size.x)
		{
			if (tab_bar->BarRect.Min.x - docked_window->Pos.x < icowidth)
				tab_bar->BarRect = ImRect(ImVec2(tab_bar->BarRect.Min.x + icowidth, tab_bar->BarRect.Min.y), ImVec2(tab_bar->BarRect.Max.x + icowidth, tab_bar->BarRect.Max.y));//平移30，留出左边图片位置

			g.NextItemData.Width = tab_bar->Tabs[0].Width;//可解决多几个tab有误差（tab右边闪）的问题，每次都让它重规则
			if (g.NextItemData.Width > 0 && g.NextItemData.Width + icowidth < docked_window->Size.x && g.NextItemData.Width != tab_bar->WidthAllTabs / tab_bar->Tabs.Size)
			{
				docked_window->DockTabItemRect.Max.x = docked_window->DockTabItemRect.Min.x + g.NextItemData.Width;
				g.LastItemData.Rect = docked_window->DockTabItemRect;
	...
	if (g.NextItemData.Width > 0 && docked_window != NULL)
	{
		size.x = tab->RequestedWidth = g.NextItemData.Width;
		tab->Width = ImMax(1.0f, size.x);
	}
	...
	//TabItemBackground(display_draw_list, bb, flags, tab_col);
	int tabtype = 1;
	DrawTabItem(display_draw_list, bb, flags, tab_col, tabtype);//改
	RenderNavHighlight(bb, id);

	ImVec2 oCursorPos=ImGui::GetIO().MousePos;
	ImRect oTabSize = tab_bar->BarRect;//tab_bar->LastTabItemIdx当前序号
	bool iscursoroninpre = oCursorPos.x<(bb.Max.x - oTabSize.GetWidth()) && oCursorPos.x>(bb.Min.x - oTabSize.GetWidth()) && oCursorPos.y<bb.Max.y&&oCursorPos.y>bb.Min.y;//光标在上一个标签上
	if (tab_bar->LastTabItemIdx>0 && (tab_bar->Tabs[tab_bar->LastTabItemIdx - 1].ID != tab_bar->SelectedTabId) && (tab_bar->SelectedTabId != id) && !hovered && !iscursoroninpre)//活动页前后都不画,光标所在页前面一个也不画,光标在上一个不是活动窗也不画
		display_draw_list->AddLine(ImVec2(bb.Min.x, bb.Min.y + 10), ImVec2(bb.Min.x, bb.Max.y - 10), ImGui::GetColorU32(ImGuiCol_TextDisabled), 1.5f);
	
	ImGuiViewport* userdata = (ImGuiViewport*)(g.IO.UserData);
	if (userdata != NULL)
	{
		if (oTabSize.GetWidth() > 30 && userdata->RendererUserData != NULL)
		{
			std::unordered_map<ImGuiID, void*>* mTextures = (std::unordered_map<ImGuiID, void*>*)userdata->RendererUserData;//要传数组进来，每个TAB都有图标
			if (mTextures)
			{
				if (mTextures->count(tab->ID)>0)
				{
					std::unordered_map<ImGuiID, void*> textures = *mTextures;
					ID3D11ShaderResourceView* mTexture = (ID3D11ShaderResourceView*)textures[tab->ID];
					display_draw_list->AddImage(mTexture, ImVec2(bb.Min.x + 5, bb.Min.y + 10), ImVec2(bb.Min.x + 21, bb.Min.y + 26), ImVec2(0, 0), ImVec2(1, 1));//第一个ImVec2和第二个ImVec2之差应是16x16不然会导致图片变形
					bb.Min.x += 21;//没图标则不需要右移
				}
			}
		}
		...
		if (held || hovered)
		{
			userdata->PlatformUserData = (void*)docked_window;
		}
	}
	...
	if (tabtype == 2)
	{
		bb.Min.x = bb.Min.x + 12;
		bb.Max.x = bb.Max.x - 12;
	}
	else
		if (tabtype == 1)
			bb.Max.x = bb.Max.x - 12;
	...
	// Render tab label, process close button
	const ImGuiID close_button_id = p_open ? GetIDWithSeed("#CLOSE", NULL, docked_window ? docked_window->ID : id) : 0;
	bool just_closed;//由TabItemLabelAndCloseButton传出是否关闭
	bool text_clipped;
	TabItemLabelAndCloseButton(display_draw_list, bb, tab_just_unsaved ? (flags & ~ImGuiTabItemFlags_UnsavedDocument) : flags, tab_bar->FramePadding, label, id, close_button_id, tab_contents_visible, &just_closed, &text_clipped);
	if (just_closed && p_open != NULL)
	{//点关闭按钮后设关闭标记
	...
	if (text_clipped && g.HoveredId == id && !held)
		if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) && !(tab->Flags & ImGuiTabItemFlags_NoTooltip))
			...
			{//统一改成用表单作Tooltip
				...
				ImGuiContext& g = *GImGui;
				char namebuf[512];
				ImFormatString(namebuf, 512, "%.*s", (int)(FindRenderedTextEnd(label) - label), label);//取出标题
				bool p_open = true;
				ImVec2 txt_size = ImGui::CalcTextSize(label, NULL, true);
				ImVec2 oCursorPos = ImGui::GetIO().MousePos;
				oCursorPos.x += 10;
				oCursorPos.y += 20;
				txt_size.x += 20;
